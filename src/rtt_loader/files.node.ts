 ...
 */
const getLoaderContent = async (
  genbaDir: string,
  csvKind: CSVKind,
): Promise<LoaderContentType> => {
 ...
 */
export const getLoaderAllContent = async (
  genbaDir: string,
): Promise<LoaderAllContentType> => {
  const [link, point, setsu, torishin] = await Promise.all(
    Object.values(CSVKind).map((csvKind) =>
      getLoaderContent(genbaDir, csvKind),
    ),
  );
 ...
 */
export const dumpGenbaJson = async (
  genba: GenbaType,
  outputPath: string,
): Promise<void> => {
