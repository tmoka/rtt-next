...
import { CSVPointRow } from '../../common/types'
import {
  CSVKind,
...
const testLoader = async (csvName: string, csvType: string): Promise<any[]> => {
  const buffer = await fs.promises.readFile(path.join('test/rtt_loader', csvName, `${csvType}.csv`))
...
describe('エラーが正しく定義されていること', () => {
  ...
  test('RTTLoaderConvertError が正しく定義されていること', () => {
    ...
    expect(error.toString()).toEqual('RTTLoaderConvertError: test message\nファイル: link.csv')
  })

  test('RTTLoaderDecodeError が正しく定義されていること', () => {
    const error = new RTTLoaderDecodeError(CSVKind.LINK, 'test message', [], [])
    expect(error).toBeInstanceOf(Error)
    expect(error).toBeInstanceOf(RTTLoaderError)
    expect(error).toBeInstanceOf(RTTLoaderDecodeError)
    expect(error.toString()).toEqual('RTTLoaderDecodeError: test message\nファイル: link.csv')
  })
  ...
  test('RTTLoaderParseError が正しく定義されていること', () => {
    ...
    expect(error.toString()).toEqual('RTTLoaderParseError: test message\nファイル: link.csv')
  })
})
