...
export const getZentai = (
  ...
  const gaihekiTorishin: GaihekiTorishinType = torishinRows.reduce(
    (obj, row) => {
      if (!row.gaihekiType) {
        return obj;
      }
      if (!isGaihekiKey(row.gaihekiType)) {
        throw new RTTLoaderConvertError(
          CSVKind.TORISHIN,
          `gaihekiType の値が不正です: '${row.gaihekiType}'。 ${Object.values(
            GaihekiKey,
          ).join(', ')} のいずれかを指定してください。`,
        );
      }
...
const getGosaVectorsAndLatestStage = (
  ...
  const gosaVectors: PartialRecord<Stage, GosaVectorType> = STAGE_ORDER.reduce(
    (acc, stage) => {
 ...
 */
export const getTorishinNames = (
  heimenList: HeimenListType,
): TorishinNamesType => {
 ...
 * @returns フォーマットの種類
 */
export const detectGenbaFormat = async (
  genbaDirPath: string,
): Promise<GenbaFormat> => {
  try {
