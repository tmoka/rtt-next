 ...
 * @returns エスケープされた節名
 */
const escapeSetsuFileName = (setsuName: string): string =>
  setsuName.replace(/\./g, '_');
 ...
 */
const asyncReadOldPointFile = async (
  filePath: string,
...
const asyncReadKihFile = async (
  ...
  const filePath = getExtCaseSensitiveFilePath(
    genbaDirPath,
    setsuName,
  ...
  const pointRows = points.map((point) => {
    ...
    const [xTorishinName, yTorishinName] = pointNameToXYToriName(
      point.pointName,
    );
    const pointRow = {
      ...defaultCSVPoint,
...
export const asyncReadTwoKihFile = async (
  ...
  const filePath1 = getExtCaseSensitiveFilePath(
    genbaDirPath,
    fileBaseName1,
    OldGenbaFileType.KIH,
  );
  const filePath2 = getExtCaseSensitiveFilePath(
    genbaDirPath,
    fileBaseName2,
    OldGenbaFileType.KIH,
  );
...
const asyncReadColFile = async (
  ...
  try {
    ...
    console.error(err); // eslint-disable-line no-console
    throw new OldGenbaToCsvError(
      `COL ファイルの内容を読み込めませんでした。 ファイル名: '${path.basename(
        filePath,
      )}'`,
    );
  }
 ...
 */
const asyncReadGaihekiFile = async (
  filePath: string,
): Promise<GaihekiTorishinType> => {
  let lines;
  try {
    lines = await asyncReadLines(filePath);
  } catch (err) {
...
const asyncReadZentaiXOrYFile = async (
  ...
  const points = await asyncReadOldPointFile(
    path.join(genbaDirPath, oldPointFileType),
  );
  const gaihekiPath = path.join(genbaDirPath, 'GAIHEKI');
  const gaihekiRow = fs.existsSync(gaihekiPath)
    ? await asyncReadGaihekiFile(gaihekiPath)
    : {};
  ...
  const torishinRows: CSVTorishinRow[] = chunk(points, 2).map(([p1, p2]) => {
    ...
    const gaihekiKeyName: string | undefined = Object.keys(
      gaihekiRow,
    ).find((key) =>
      isGaihekiKey(key) && gaihekiRow[key] === name1
        ? gaihekiRow[key]
        : undefined,
    );
...
const asyncReadLinkFile = async (
  ...
  try {
    ...
    console.error(err); // eslint-disable-line no-console
    throw new OldGenbaToCsvError(
      `LINK ファイルの内容を読み込めませんでした。 ファイル名: '${path.basename(
        filePath,
      )}'`,
    );
  }
 ...
 * @returns
 */
const asyncReadZentaiZFile = async (
  genbaDirPath: string,
): Promise<CSVSetsuRow[]> => {
  ...
  lines.forEach((buf, i) => {
    ...
    if (line % 2 === 0) {
      ...
      if (!setsuName) {
        throw new OldGenbaToCsvError(
          `節名を識別できませんでした。 ファイル名: '${path.basename(
            filePath,
          )}' , 行数: '${i + 1}'行目`,
        );
      }
 ...
 */
export const kihToPointCsv = async (
  genbaDirPath: string,
): Promise<CSVPointRow[]> => {
  ...
  const kihData = await Promise.all(
    setsuNames.map(async (setsuName) =>
      asyncReadKihFile(genbaDirPath, setsuName),
    ),
  );
...
const pointsToCsv = async (genbaDirPath: string): Promise<CSVPointRow[]> => {
  ...
  await asyncForEach(OLD_GOSA_FILE_TYPES, async (fileType) => {
    ...
    const gosaData = await Promise.all(
      setsuNames.map(async (setsuName) =>
        asyncReadGosaFile(genbaDirPath, setsuName, fileType),
      ),
    );
    ...
    gosaData.forEach((gosaPoints, i) => {
      gosaPoints.forEach((gosaPoint) => {
        const pointRow = pointsData[i].find(
          (row) => row.pointName === gosaPoint.pointName,
        );
        if (pointRow) {
          pointRow[oldPointFileTypeToGosaKey(fileType, AXIS.X)] =
            gosaPoint.x - pointRow.kihX;
          pointRow[oldPointFileTypeToGosaKey(fileType, AXIS.Y)] =
            gosaPoint.y - pointRow.kihY;
          pointRow[oldPointFileTypeToGosaKey(fileType, AXIS.Z)] =
            gosaPoint.z - pointRow.kihZ;
        }
      });
    });
  });
  ...
  const colData = await Promise.all(
    setsuNames.map(async (setsuName) =>
      asyncReadColFile(genbaDirPath, setsuName),
    ),
  );
...
const linkToCsv = async (genbaDirPath: string): Promise<CSVLinkRow[]> => {
  ...
  const linkData = await Promise.all(
    setsuNames.map(async (setsuName) =>
      asyncReadLinkFile(genbaDirPath, setsuName),
    ),
  );
...
export const transformKihWithBase = (
  ...
  const base0 = pointRows.find(
    (pointRow) => pointRow.pointName === BASE0_POINT_NAME,
  );
  if (!base0) {
    throw new OldGenbaToCsvError(
      `基準となるポイント "${BASE0_POINT_NAME}" がありません。`,
    );
  }
  ...
  // 角度を求める
  const base1 = translatedPointRows.find(
    (pointRow) => pointRow.pointName === BASE1_POINT_NAME,
  );
  if (!base1) {
    throw new OldGenbaToCsvError(
      `基準となるポイント "${BASE1_POINT_NAME}" がありません。`,
    );
  }
  ...
  await asyncForEach(
    ...
    async (stageIdx): Promise<void> => {
      ...
      stagePointRows.forEach((stagePointRow) => {
        const targetRow = pointRows.find(
          (pointRow) => pointRow.pointName === stagePointRow.pointName,
        );
        if (!targetRow) {
          throw new OldGenbaToCsvError(
